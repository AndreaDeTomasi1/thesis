...
The loss function is the mathematical criterion that guides the training of the retriever. It is typically of a contrastive type: given a set of relevant (positive) and non-relevant (negative) query-document pairs, the loss is defined to maximize the similarity between the vectors of positive pairs and minimize that of negative pairs. Formally, the parameters of the encoder are optimized so that the geometric structure of the vector space reflects a notion of semantic relevance. In this way, the learned function induces a metric in the embedding space that approximates the concept of relevance to the query.

The most relevant documents identified by the retriever are provided as context to the generative model, which uses them to produce the response. This integration between semantic retrieval and text generation allows for enriching the parametric knowledge of the model with updatable external information, improving performance on knowledge-intensive tasks.
...
At the same time, recent studies emphasize the importance of managing the context window of LLMs. Techniques for semantic compression, adaptive document selection, and noise control aim to optimize the use of available context. In particular, Cuconasu et al. (2024) show that appropriate filtering and weighting of retrieved documents reduce noise and improve model effectiveness @noise2024, while Cheng et al. (2025) provide a systematic overview of the latest strategies for efficiently combining and selecting documents @survey2025.

An intuitive result that has emerged in the literature concerns the role of noise in the retrieval process. Cuconasu et al. (2024) show that the controlled introduction of non-perfectly relevant documents can improve the accuracy of the generative model, suggesting that a more diversified context favors the inferential process @noise2024. A possible theoretical conjecture supporting this result is that the presence of moderately heterogeneous information acts as a form of contextual regularization: the inclusion of documents not strictly overlapping with the query could reduce the risk of model overfitting to partial or redundant evidence, promoting a more robust latent representation. Additionally, a diversified context could expand the semantic space explored by the model during generation, increasing the likelihood of activating relevant but not immediately evident conceptual connections.

Another hypothesis is that "controlled noise" improves attention calibration: the need to discriminate between strongly and weakly relevant signals could prompt the model to weigh available information more selectively, strengthening integration and comparison mechanisms between sources. In this perspective, the observed positive effect would not derive from the noise itself, but from its function as a stimulus for selection and inferential composition within the context window. This result challenges traditional retrieval evaluation metrics and opens new perspectives in the design of RAG systems.
...
The processing is also guided by a shared "graph state," which is a data structure containing relevant information accumulated during execution. 
...
Using a graph-based architecture allows for implementing more elaborate reasoning strategies, such as iterations between retrieval and generation, quality checks on produced responses, or fallback mechanisms when retrieved information is insufficient. 
...
If the response is deemed satisfactory, the system terminates the conversation, returning a contextualized response to the end user based on external sources, thereby improving the reliability and relevance of the information provided compared to a standalone generative model.
...
Additionally, all steps are accompanied by "reasoning," which is a textual explanation that describes the decisions made by the system at each stage, thus improving the transparency and interpretability of the process.
...
After this phase, the documents are sorted based on relevance and filtered to retain only the most pertinent ones through a "sigmoid" centered on the average scores with steepness 8, in order to reduce noise and improve the quality of the context provided to the generative model. Only documents with a relevance score above 0.5 are retained, while those with a score below 0.5 are discarded, as shown in the @fig:sigmoid.
...
The strengths of the system include the modularity of the architecture, which allows for replacing or updating individual components without altering the entire workflow, and the ability to dynamically integrate external information, improving the factuality and transparency of the generated responses. Additionally, the use of a graph-based approach has allowed for implementing more elaborate reasoning strategies and maintaining a complete record of the interaction, facilitating subsequent iterations and post-hoc analyses of the system's performance.

Among future developments, there is a plan to explore integration with existing healthcare systems, in order to further improve the relevance and utility of the responses provided to AREU operators. Another research direction involves exploring multimodality, integrating non-textual knowledge sources such as images or structured data to further enrich the context provided to the generative model. Finally, there are plans to conduct a systematic comparison between different embedding and generation models to identify the most effective configurations for the specific context of AREU, and to explore hierarchical ranking strategies to further optimize the selection of documents during the retrieval process.
