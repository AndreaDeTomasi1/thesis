...
The system is orchestrated using LangGraph in conjunction with DecAgents, a proprietary library of Ottante SRL (part of the Beta 80 group). By leveraging the core functionalities and networking capabilities of DecAgents—a library already hosted in several cloud-based production environments—the architecture overcomes the linearity of traditional pipelines by leveraging a computational graph in which specialized autonomous agents collaborate on query decomposition, semantic retrieval, and result validation.
